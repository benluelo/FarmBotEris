...
export default class Log {
     ...
     */
    static default(...args) {
        console.log(chalk.white.bold(`${basename(get()[1].getFileName())}:`), chalk.cyan(`[LOG][DEFAULT][${currentTimestamp()}]`), args.join(" "));
    }
    /**
     ...
     */
    static connect(...args) {
        console.log(chalk.white.bold(`${basename(get()[1].getFileName())}:`), chalk.green(`[LOG][CONNECT][${currentTimestamp()}]`), args.join(" "));
    }
    /**
    ...
    static dbconnect(...args) {
        console.log(chalk.white.bold(`${basename(get()[1].getFileName())}:`), chalk.keyword("lime")(`[LOG][DBCONNECT][${currentTimestamp()}]`), args.join(" "));
    }
    /**
    ...
    static commandLoad(...args) {
        console.log(chalk.white.bold(`${basename(get()[1].getFileName())}:`), chalk.keyword("cyan")(`[LOG][CMD LOAD][${currentTimestamp()}]`), args.join(" "));
    }
    /**
    ...
    static directoryLoad(...args) {
        console.log(chalk.white.bold(`${basename(get()[1].getFileName())}:`), chalk.blue(`[LOG][DIR LOAD][${currentTimestamp()}]`), args.join(" "));
    }
    /**
    ...
    static error(...args) {
        ...
        console.log(chalk.white.bold(`${errorFile}:`), chalk.red(`[LOG][ERROR][${currentTimestamp()}]`), chalk.red.bold(`${fileName}:`), args.join(" "));
        fs.writeFileSync(join(process.cwd(), "/bot/logs/errors/", fileName), `${errorFile}\n${args.join("\n")}`);
        console.log(chalk.white.bold(`${errorFile}:`), chalk.red.bold(`${fileName}:`), "Saved!");
    }
    /**
    ...
    static toFile(...args) {
        const errorFile = basename(get()[1].getFileName());
        console.log(chalk.white.bold(`${errorFile}:`), chalk.yellow(`[LOG][TO FILE][${currentTimestamp()}]`), args.join(" "));
