...
export function run(bot: FarmBotClient): void {
  const cmd = bot.addCommand('plant', async (message, [plot, crop, ..._args], userdata) => {
    ...
    if (plot === undefined) { return message.send(new Embed().uhoh('You have to specify a plot to plant on!')); }
    if (crop !== undefined && plot !== undefined) {

      // check if input is valid
      const plotNumber = parsePlotNumber(plot);
      if (!cropData[crop as CropName] || !userdata.seeds.common[crop as CropName].discovered) {
      ...
      if (undefined !== plotNumber) {
        ...
        return message.send(new Embed().success(`Planted ${cropData[crop as CropName].emoji} on \`${plot.charAt(0).toUpperCase() + plot.slice(1)}\`!`));
      } else {
  ...
  cmd.subcommand('all', async (message, args, userdata) => {
    ...
    if (!crop) { return message.send(new Embed().uhoh('Please specify the crop you want to plant!')); }
    if (!cropData[crop as CropName] || !userdata.seeds.common[crop as CropName].discovered) { return message.send(new Embed().uhoh(`Couldn't find **${crop}** seeds in your seedbag... maybe you mispelled it?`)); }
    ...
    return await msg.edit({
      content: '',
      ...new Embed().success(`Successfully planted **${totalPlots}** ${cropData[crop as CropName].emoji}!`)
    });
  }, {
  ...
  cmd.subcommand('row', async (message, [row, crop, ..._args], userdata) => {
    ...
    function isValidRow(r: string): r is keyof typeof rows {
      return Object.prototype.hasOwnProperty.call(rows, r);
    }
    ...
    if (userdata.farm.length < rows[row]) { return message.send(new Embed().uhoh(`You don't own any plots on row **${row.toUpperCase()}**`)); }
