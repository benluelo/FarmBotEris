...
function parseWants(preferences, request) {
    ...
    for (const w in request.want) {
        const flavorMultiplier = [...new Set([...cropData[request.want[w].crop].flavour, ...preferences.taste])].length * 0.15;
        const colorMultiplier = cropData[request.want[w].crop].color === preferences.color ? 0.15 : 0;
        const totalMultiplier = 1 + flavorMultiplier + colorMultiplier;
        parsed.value += ((seedsPrice[request.want[w].crop] * request.want[w].amount * request.value) * totalMultiplier);
