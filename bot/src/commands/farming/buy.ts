...
export function run(bot: FarmBotClient): void {
  bot.addCommand('buy', (message, _args, userdata) => {
    ...
    const priceOfNextPlotEmbed = new Embed()
      .setTitle('')
      .setDescription(`The next plot costs **${formatMoney(priceOfNextPlot)}!**`)
    ...
    return message.send(new Embed()
      .setDescription(`Plot purchased for **${formatMoney(priceOfNextPlot)}**! You now own ${res.value.farm.length + 1} plots!${res.value.farm.length + 1 === MAX_PLOTS ? ' This is the maximum amount of plots!' : ''}`)
