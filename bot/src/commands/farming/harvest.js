            ...
            if (plotNumber !== undefined) {
                if (plotNumber >= userdata.farm.length) {
                    ...
                    if (userCrop.planted === "dirt") {
                        return message.send(new Embed().uhoh(`There's nothing on plot #\`${plotToHarvest.toUpperCase()}\` to harvest!`));
                    }
                    if (((Date.now() - userCrop.datePlantedAt) >= bot.config.farminfo.growTimes[userCrop.planted])) {
                        ...
                        message.send(new Embed().uhoh(`The ${cropData[userdata.farm[plotNumber].crop.planted].emoji} on  plot #\`${plotToHarvest.toUpperCase()}\` hasn't finished growing yet!`));
                    }
                }
            }
        }
