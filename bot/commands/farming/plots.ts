  ...
  bot.addCommand("plots", (message, _args, userdata) => {
    ...
    message.send("Building farm...").then((msg) => {
      for (const plot in userFarm) {
        ...
        if ((plot as unknown as number) % 5 === 0) {
        ...
        if (userFarm[plot].crop.planted === "dirt") { // if dirt, add dirt (lol)
          plotsMsg += emoji.dirt
        } else if (Date.now() - userFarm[plot as unknown as number].crop.datePlantedAt >= bot.config.farminfo.growTimes[userFarm[plot].crop.planted as CropName]) { // if not dirt, and if the crop is grown, add the crop
          plotsMsg += cropData[userFarm[plot].crop.planted as CropName].emoji
