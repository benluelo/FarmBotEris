 ...
 */
export default function run(region) {
    ...
    const fruitsArray = Object.keys(cropData)
        .filter((name) => "apple" !== name);
    return [...new Array(11)]
        .map(() => Math.random())
        .sort()
        .map((num, i) => new NPC(tr(nameSurnameAndGenderArray[i].name + " " + nameSurnameAndGenderArray[i].surname), nameSurnameAndGenderArray[i].gender, fruitsArray[i], num));
}
